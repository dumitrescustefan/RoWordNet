...
class WordNet(object):
    ...
    def _load_from_binary(self, filename: str):
        ...
        for synset_id in synsets_id:
            adj_synsets_id = wn.adjacent_synsets(synset_id, show_relations=True)
    ...
    def adjacent_synsets(self, synset_id: str, relation: str=None, show_relations: bool=False):
        """
            ...
            Args:
                ...
                show_relations (str, optional): If set to True, it will show the relations between synsets. Defaults to
                    False.
            Returns:
        ...
        if relation is not None and show_relations is not False:
            raise ValueError("Arguments 'relation' and 'show_relations' can't be both valid")
        ...
        try:
            if show_relations is False:
                if relation is None:
                    adj_synsets_id = [adj_synset_id for adj_synset_id in self._graph[synset_id]]
                else:
                ...
                if not isinstance(show_relations, bool):
                    raise TypeError("Argument 'show_relations' has incorrect type, expected bool, got {}"
                                    .format(type(show_relations).__name__))

                for adj_synset_id, data in self._graph.adj[synset_id].items():
                    adj_synsets_id.append((adj_synset_id, data['label']))
        except KeyError:
            return []
    ...
    def lowest_hypernym_common_ancestor(self, synset_id1: str, synset_id2: str):
        ...
        synset_id1_to_root = self.synset_to_root(synset_id1)
        synset_id2_to_root = self.synset_to_root(synset_id2)
